...
const UsageExportHeader: React.FC<UsageExportHeaderProps> = ({
  ...
  const emptyPlaceholder = t("usage:exportHeader.noEntityUsage", {
    entity: t(`usage:entity.typePlural.${entityType}`),
  });
  ...
  const filterList = (
    <ComboboxContent anchor={anchor}>
      <ComboboxEmpty>{t("usage:exportHeader.noOptions")}</ComboboxEmpty>
      <ComboboxList>
        {(value: string) => (
    ...
    >
      <ComboboxChips render={<div ref={anchor} />} className="w-full">
        ...
        {selectedFilters.length > 0 && (
          <ComboboxClear
            aria-label={t("usage:exportHeader.clearFilters", {
              filter: filterLabel ?? t("usage:exportHeader.filtersNoun"),
            })}
          />
        )}
      </ComboboxChips>
