...
const EndpointUsageTable: React.FC<EndpointUsageTableProps> = ({ endpointData }) => {
  ...
  const columns: ColumnDef<EndpointRow>[] = [
    {
      header: t("usage:endpoint.columnEndpoint"),
      accessorKey: "endpoint",
      cell: ({ row }) => <span className="font-medium">{row.original.endpoint}</span>,
    },
    {
      header: t("usage:endpoint.columnSuccessFailed"),
      id: "requests",
      cell: ({ row }) => {
    ...
    {
      header: t("usage:endpoint.columnTotalRequests"),
      accessorKey: "api_requests",
      meta: { numeric: true },
      cell: ({ row }) => row.original.api_requests.toLocaleString(),
    },
    {
      header: t("usage:endpoint.columnSuccessRate"),
    ...
    {
      header: t("usage:page.totalTokens"),
  ...
  return (
    <DataTable
      ...
      noDataMessage={t("usage:endpoint.noData")}
      size="compact"
    />
  );
};
