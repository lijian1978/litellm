  ...
  return (
    <div className="w-full space-y-6">
      <div className="flex flex-wrap items-center justify-end gap-4">
        <span className="text-sm text-muted-foreground">{t("cost:optimization.usageTab.utcBucketed")}</span>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          ...
          <CardHeader>
            <CardTitle>{t("cost:optimization.usageTab.savings")}</CardTitle>
            ...
            <CardAction className="flex flex-wrap items-center justify-end gap-x-4 gap-y-2">
              ...
              <Tabs value={accumulation} onValueChange={(value) => setAccumulation(value as SavingsAccumulation)}>
                <TabsList>
                  <TabsTrigger value="cumulative">{t("cost:optimization.usageTab.cumulative")}</TabsTrigger>
        ...
        <Card>
          <CardHeader>
            <CardTitle>{t("cost:optimization.usageTab.savingsByDriver")}</CardTitle>
          </CardHeader>
          <CardContent>
            <DonutChart
              className="h-80"
          ...
          <CardContent>
            {topTools.length === 0 ? (
              <p className="py-8 text-center text-sm text-muted-foreground">
                {toolSpendLoading ? t("common:loading.ellipsis") : t("cost:optimization.usageTab.noToolUsage")}
              </p>
            ) : (
