      ...
      <HoverCardContent align="start" className="w-80">
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-0.5">
            <span className="text-xs text-muted-foreground">{t("models:columns.publicModelName")}</span>
            <span className="truncate text-sm font-medium text-foreground" title={displayName}>
              {displayName}
            </span>
          </div>
          <div className="flex flex-col gap-0.5">
            <span className="text-xs text-muted-foreground">{t("models:columns.litellmModelName")}</span>
            <span className="flex min-w-0 items-center gap-1.5">
              ...
              <button
                type="button"
                aria-label={t("models:columns.copyLiteLLMNameAria")}
                data-testid={`copy-litellm-model-name-${model.model_info.id}`}
                className="shrink-0 cursor-pointer text-muted-foreground hover:text-foreground"
                onClick={() => void copyToClipboard(litellmModelName, t("models:columns.copyLiteLLMNameSuccess"))}
              >
      ...
      <HoverCard>
        <HoverCardTrigger
          render={
            <button
              type="button"
              aria-label={t("models:credentials.aboutAria")}
        ...
        <HoverCardContent align="start" className="w-80">
          <div className="flex flex-col gap-3">
            <span className="text-sm font-medium text-foreground">{t("models:credentials.types")}</span>
            <div className="flex flex-col gap-1">
              <span className="flex items-center gap-1.5 text-sm font-medium text-info">
                <RefreshCw className="size-3.5" />
                {t("models:credentials.reusable")}
              </span>
              <span className="text-xs text-muted-foreground">{t("models:credentials.reusableDesc")}</span>
            </div>
            <div className="flex flex-col gap-1">
              <span className="flex items-center gap-1.5 text-sm font-medium text-foreground">
                <Pencil className="size-3.5" />
                {t("models:credentials.manual")}
              </span>
              <span className="text-xs text-muted-foreground">{t("models:credentials.manualDesc")}</span>
            </div>
          </div>
        </HoverCardContent>
      </HoverCard>
...
function CreatedByCell({ model }: { model: ModelData }) {
  ...
  const primary = isConfigModel
    ? t("models:columns.definedInConfig")
    : model.model_info.created_by || t("models:columns.unknown");
  const secondaryForDbModel = createdAt ?? t("models:columns.unknownDate");
...
function CostsCell({ model }: { model: ModelData }) {
  ...
  return (
    <CellTooltip
      content={t("models:columns.costPer1MTokens")}
      trigger={
        ...
        <CellTooltip
          ...
          trigger={
            <Badge variant="outline" className="shrink-0 cursor-default font-normal">
              {t("models:columns.more", { count: overflow.length })}
            </Badge>
          }
        />
      )}
...
function ModelRowActions({
  ...
  const resolvePauseTooltip = (): string => {
    if (isConfigModel) {
      return t("models:pause.configCannotPause");
    }
    if (!isAdmin) {
      return t("models:pause.adminOnly");
    }
    return isBlocked
      ? t("models:pause.resumeTooltip")
      : t("models:pause.pauseTooltip");
  };

  const deleteTooltip = isConfigModel
    ? t("models:delete.configCannotDelete")
            ...
            trigger={
              <span className="inline-flex">
                <Switch
                  ...
                  aria-label={isBlocked ? t("models:pause.resumeAria") : t("models:pause.pauseAria")}
...
export const getModelsTableColumns = ({
  ...
  },
  {
    ...
    meta: { title: t("models:columns.modelInformation"), skeleton: "twoLine" },
    header: ({ column }) => <DataTableSortHeader column={column} title={t("models:columns.modelInformation")} />,
    enableSorting: true,
    size: 280,
    minSize: 160,
    cell: ({ row }) => (
  ...
  },
  {
    ...
    cell: ({ row }) =>
      row.original.model_info.db_model ? (
        <StatusBadge tone="info" label={t("models:columns.dbModel")} />
      ) : (
        <StatusBadge tone="neutral" label={t("models:columns.configModel")} />
      ),
  },
  {
    id: "actions",
