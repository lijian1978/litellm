...
const VIEW_MODE_LABEL_KEYS: Record<ModelViewMode, string> = {
  current_team: "models:table.viewMode.currentTeam",
  all: "models:table.viewMode.all",
};
...
function EmptyState() {
  ...
  return (
    <div className="flex flex-col items-center gap-1 py-6">
      ...
      <div className="text-base font-semibold text-foreground">{t("models:table.empty.noModels")}</div>
      <div className="max-w-80 text-sm text-muted-foreground">{t("models:table.empty.noModelsHint")}</div>
    </div>
  );
}
...
export function AllModelsTable({
  ...
  }, [userRole, userID, onModelIdClick, onTeamIdClick, onDeleteClick, onTogglePauseClick, pausingModelId, t]);

  const modelGroupOptions = useMemo(
    () => [
      { label: t("models:table.group.allModels"), value: ALL_MODEL_GROUPS_VALUE },
      { label: t("models:table.group.wildcard"), value: WILDCARD_MODEL_GROUP_VALUE },
  ...
  const accessGroupOptions = useMemo(
    () => [
      { label: t("models:table.group.allAccessGroups"), value: ALL_MODEL_GROUPS_VALUE },
  ...
  const formatFilterValue = (columnId: string, value: unknown): string => {
    ...
    if (columnId === MODEL_NAME_COLUMN_ID && raw === WILDCARD_MODEL_GROUP_VALUE) {
      return t("models:table.group.wildcard");
    }
    return raw;
  };
  ...
  return (
    <DataTable
      ...
      isLoading={isLoading}
      loadingMessage={t("models:table.loading")}
      ...
      toolbar={(table) => (
        <>
          <DataTableToolbar
            ...
            filterLabels={{
              [MODEL_NAME_COLUMN_ID]: t("models:table.filter.publicModelName"),
              [ACCESS_GROUPS_COLUMN_ID]: t("models:table.filter.accessGroup"),
            }}
          ...
          >
            <Select value={selectedTeamValue} onValueChange={(value) => onTeamChange(String(value))}>
              <SelectTrigger
                size="sm"
                aria-label={t("models:table.teamAria")}
              ...
              >
                ...
                />
                <span className="text-muted-foreground">{t("models:table.team")}</span>
            ...
            <Select value={viewMode} onValueChange={(value) => onViewModeChange(value as ModelViewMode)}>
              <SelectTrigger size="sm" aria-label={t("models:table.viewAria")} data-testid="models-view-select" className="gap-2">
                <span className="text-muted-foreground">{t("models:table.view")}</span>
            ...
            <Button
              variant="outline"
              size="icon-sm"
              aria-label={t("models:table.settingsAria")}
              title={t("models:table.settingsTitle")}
          ...
          <DataTableFilterDrawer
            ...
            title={t("models:table.filters.title")}
            description={t("models:table.filters.description")}
            resetLabel={t("models:table.filters.reset")}
            onReset={onResetFilters}
          >
            {({ get, set }) => (
              <>
                <DataTableFilterField label={t("models:table.filter.publicModelName")}>
                  <SearchSelect
                    ...
                    placeholder={t("models:table.filter.byPublicModelName")}
                    emptyText={t("models:table.empty.noModels")}
                  />
                </DataTableFilterField>
                <DataTableFilterField label={t("models:table.filter.accessGroup")}>
                  <SearchSelect
                    ...
                    placeholder={t("models:table.filter.byAccessGroup")}
                    emptyText={t("models:table.filter.noAccessGroups")}
                  />
                </DataTableFilterField>
              </>
            )}
