...
export function AutoRoutersPanel({
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      toast.success(t("models:autoRouter.deletedToast", { name: deletingRouter.name }));
      setDeletingRouter(null);
      await invalidateAutoRouters();
    } catch (error) {
      toast.fromError(t("models:autoRouter.deleteFailedToast", { error: String(error) }));
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  return (
    <div className="w-full space-y-4">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-base font-semibold text-foreground">{t("models:autoRouter.heading")}</h2>
          <p className="mt-1 text-sm text-muted-foreground">{t("models:autoRouter.description")}</p>
        </div>
        {canCreate && (
          <Button onClick={() => setIsCreating(true)} className="shrink-0">
            <Plus />
            {t("models:autoRouter.add")}
          </Button>
        )}
      </div>
      ...
      <Dialog open={isCreating} onOpenChange={setIsCreating}>
        ...
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-4xl">
          <DialogHeader>
            <DialogTitle>{t("models:autoRouter.add")}</DialogTitle>
            <DialogDescription>{t("models:autoRouter.addDescription")}</DialogDescription>
          </DialogHeader>
          <AddAutoRouterTab
      ...
      {deletingRouter && (
        <DeleteResourceModal
          isOpen
          title={t("models:autoRouter.deleteTitle")}
          message={t("models:autoRouter.deleteMessage", { name: deletingRouter.name })}
          resourceInformationTitle={t("models:autoRouter.resourceTitle")}
          resourceInformation={[
            { label: t("models:autoRouter.nameLabel"), value: deletingRouter.name },
            { label: t("models:autoRouter.typeLabel"), value: deletingRouter.typeLabel },
