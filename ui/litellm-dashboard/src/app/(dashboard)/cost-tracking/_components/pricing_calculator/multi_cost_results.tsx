...
const SingleModelBreakdown: React.FC<{
  ...
  const periodLabel = t(timePeriod === "day" ? "cost:calculator.daily" : "cost:calculator.monthly");
    ...
    <div className="space-y-3 bg-muted p-4 rounded-lg">
      {loading && (
        <div className="flex items-center gap-2 text-muted-foreground text-sm">
          ...
          <span>{t("cost:calculator.updating")}</span>
        </div>
      )}

      <div className="grid grid-cols-4 gap-4">
        <div className="min-w-0">
          <p className="text-xs text-muted-foreground block">{t("cost:calculator.totalPerRequest")}</p>
          <p className="text-base font-semibold text-info break-words">{formatCost(result.cost_per_request)}</p>
        </div>
        <div className="min-w-0">
          <p className="text-xs text-muted-foreground block">{t("cost:calculator.inputCost")}</p>
          <p className="text-sm break-words">{formatCost(result.input_cost_per_request)}</p>
        </div>
        <div className="min-w-0">
          <p className="text-xs text-muted-foreground block">{t("cost:calculator.outputCost")}</p>
          <p className="text-sm break-words">{formatCost(result.output_cost_per_request)}</p>
        </div>
        <div className="min-w-0">
          <p className="text-xs text-muted-foreground block">{t("cost:calculator.marginFee")}</p>
        ...
        <div className="grid grid-cols-4 gap-4 pt-2 border-t border-border">
          <div className="min-w-0">
            <p className="text-xs text-muted-foreground block">
              {periodLabel} {t("cost:calculator.total")} ({formatRequests(periodRequests)} {t("cost:calculator.req")})
            </p>
            <p
      ...
      {(result.input_cost_per_token || result.output_cost_per_token) && (
        <div className="text-xs text-muted-foreground pt-2 border-t border-border">
          {t("cost:calculator.tokenPricing")}{" "}
          {result.input_cost_per_token && (
            <span>
              {t("cost:calculator.input")} ${formatNumberWithCommas(result.input_cost_per_token * 1_000_000, 2)}
              {t("cost:calculator.perMillion")}
            </span>
          )}
          {result.input_cost_per_token && result.output_cost_per_token && " | "}
          {result.output_cost_per_token && (
            <span>
              {t("cost:calculator.output")} ${formatNumberWithCommas(result.output_cost_per_token * 1_000_000, 2)}
              {t("cost:calculator.perMillion")}
            </span>
          )}
        </div>
  ...
  if (!hasAnyResult && !isAnyLoading && !hasAnyError) {
    return (
      <div className="py-6 text-center border border-dashed border-border rounded-lg bg-muted">
        <p className="text-muted-foreground">{t("cost:calculator.empty")}</p>
      </div>
    );
  }
  ...
  if (!hasAnyResult && isAnyLoading && !hasAnyError) {
    return (
      <div className="py-6 text-center">
        ...
        <p className="text-muted-foreground block mt-2">{t("cost:calculator.calculating")}</p>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <p className="text-base font-semibold text-foreground">{t("cost:calculator.costEstimates")}</p>
        ...
        {errorEntries.map((e) => (
          ...
          >
            <span className="font-medium">{e.entry.model || t("cost:calculator.unknownModel")}: </span>
            {e.error}
          </div>
        ))}
      </div>
    ...
    <div className="space-y-4">
      ...
      <Card size="sm" className="px-4 bg-linear-to-r from-slate-50 to-blue-50 dark:from-slate-900 dark:to-blue-950">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2">
          <div className="min-w-0">
            <span className="text-xs text-muted-foreground">{t("cost:calculator.totalPerRequestCap")}</span>
          ...
          <div className="min-w-0">
            <span className="text-xs text-muted-foreground">
              {t("cost:calculator.totalPeriod", { period: periodLabel })}
            </span>
            <div
        ...
        {hasMargin && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2 mt-3 pt-3 border-t border-border">
            <div className="min-w-0">
              <div className="text-xs text-muted-foreground">{t("cost:calculator.marginFeePerRequest")}</div>
            ...
            <div className="min-w-0">
              <div className="text-xs text-muted-foreground">
                {t("cost:calculator.periodMarginFee", { period: periodLabel })}
              </div>
              <div className="text-sm font-mono text-warning break-words">
      ...
      {summaryData.length > 0 && (
        <Table className="border border-border rounded-lg">
          <TableHeader>
            <TableRow>
              <TableHead>{t("cost:pricingCalculator.model")}</TableHead>
              <TableHead className="text-right">{t("cost:calculator.perRequest")}</TableHead>
              <TableHead className="text-right">{t("cost:calculator.marginFeeCol")}</TableHead>
              <TableHead className="text-right">{periodLabel}</TableHead>
              <TableHead className="w-10">
                <span className="sr-only">{t("cost:calculator.costBreakdown")}</span>
              </TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
                      ...
                      <div className="flex min-w-0 flex-col gap-1">
                        ...
                        {record.hasZeroCost && !record.error && (
                          <div className="text-xs text-warning bg-warning/10 px-2 py-1 rounded-sm">
                            ⚠️ {t("cost:calculator.noPricingData")}
                          </div>
                        )}
                      </div>
                    </TableCell>
