...
type ExportTranslate = (key: string, options?: Record<string, unknown>) => string;
...
const generateModelSection = (result: CostEstimateResponse, t: ExportTranslate): string => {
  return `
    <div class="model-section">
      <h3>${result.model} ${result.provider ? `<span class="provider">(${result.provider})</span>` : ""}</h3>

      <div class="meta">
        <p><strong>${t("cost:export.inputTokensPerRequest")}:</strong> ${formatRequestsForExport(result.input_tokens)}</p>
        <p><strong>${t("cost:export.outputTokensPerRequest")}:</strong> ${formatRequestsForExport(result.output_tokens)}</p>
        ${result.num_requests_per_day ? `<p><strong>${t("cost:export.requestsPerDay")}:</strong> ${formatRequestsForExport(result.num_requests_per_day)}</p>` : ""}
        ${result.num_requests_per_month ? `<p><strong>${t("cost:export.requestsPerMonth")}:</strong> ${formatRequestsForExport(result.num_requests_per_month)}</p>` : ""}
      </div>

      <table>
        <tr>
          <th>${t("cost:export.costType")}</th>
          <th>${t("cost:export.perRequest")}</th>
          ${result.daily_cost !== null ? `<th>${t("cost:export.daily")}</th>` : ""}
          ${result.monthly_cost !== null ? `<th>${t("cost:export.monthly")}</th>` : ""}
        </tr>
        <tr>
          <td>${t("cost:export.inputCost")}</td>
        ...
        <tr>
          <td>${t("cost:export.outputCost")}</td>
...
export const exportMultiToPDF = (multiResult: MultiModelResult, t: ExportTranslate): void => {
  const printWindow = window.open("", "_blank");
  if (!printWindow) {
    window.alert(t("cost:export.allowPopups"));
    return;
  }
  ...
  const html = `
    ...
    <html>
    <head>
      <title>${t("cost:export.windowTitle")}</title>
      <style>
        body {
    ...
    <body>
      <h1>${t("cost:export.reportTitle")}</h1>
      <p style="color: #666; margin-top: -20px; margin-bottom: 30px;">${t("cost:export.modelsConfigured", { count: modelCount })}</p>

      <div class="summary-box">
        <h2>${t("cost:export.combinedTotals")}</h2>
        <div class="summary-grid">
          <div class="summary-item">
            <div class="label">${t("cost:calculator.totalPerRequest")}</div>
            <div class="value blue">${formatCostForExport(multiResult.totals.cost_per_request)}</div>
          </div>
          <div class="summary-item">
            <div class="label">${t("cost:export.totalDaily")}</div>
            <div class="value green">${formatCostForExport(multiResult.totals.daily_cost)}</div>
          </div>
          <div class="summary-item">
            <div class="label">${t("cost:export.totalMonthly")}</div>
      ...
      <h2>${t("cost:export.modelBreakdown")}</h2>
      ${validEntries.map((e) => generateModelSection(e.result!, t)).join("")}

      <div class="footer">
        <p>${t("cost:export.generatedBy", { datetime: new Date().toLocaleString() })}</p>
      </div>
    </body>
    </html>
  `;
...
export const exportMultiToCSV = (multiResult: MultiModelResult, t: ExportTranslate): void => {
  ...
  // Summary section
  rows.push(
    [t("cost:export.combinedTotals")],
    [t("cost:calculator.totalPerRequest"), multiResult.totals.cost_per_request.toString()],
    [t("cost:export.totalDaily"), multiResult.totals.daily_cost?.toString() || "-"],
    [t("cost:export.totalMonthly"), multiResult.totals.monthly_cost?.toString() || "-"],
    [t("cost:export.marginPerRequest"), multiResult.totals.margin_per_request.toString()],
    [t("cost:export.dailyMargin"), multiResult.totals.daily_margin?.toString() || "-"],
    [t("cost:export.monthlyMargin"), multiResult.totals.monthly_margin?.toString() || "-"],
    [""],
  );
  ...
  rows.push([
    t("cost:export.model"),
    t("cost:export.provider"),
    t("cost:export.inputTokens"),
    t("cost:export.outputTokens"),
    t("cost:export.requestsPerDayShort"),
    t("cost:export.requestsPerMonthShort"),
    t("cost:export.costPerRequest"),
    t("cost:export.dailyCost"),
    t("cost:export.monthlyCost"),
    t("cost:export.inputCostPerReq"),
    t("cost:export.outputCostPerReq"),
    t("cost:export.marginPerReq"),
  ]);
